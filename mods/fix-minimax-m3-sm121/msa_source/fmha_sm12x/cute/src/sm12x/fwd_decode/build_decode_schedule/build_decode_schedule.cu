...
__global__ void build_decode_schedule_gpu_kernel(
  ...
  {
    // Per-thread: read kv_pages, write to gmem + shmem.
    int kv_pages_b = 0;
    if (tid < batch) {
    ...
    // warp partials with another shuffle reduce.  The two-level reduce is
    // required for batch > 32 (more than one warp of per-batch slots).
    ...
    if (warp == 0) {
      ...
      // Split-KV thresholds.  Splitting a batch into KV chunks adds a combine
      // pass (extra O_partial / LSE_partial fp32 writes, ~5-10us); below these
      // sizes that overhead outweighs the added parallelism:
      //   kMinUsefulChunkPages = 16  chunk floor (~2K tokens); smaller chunks
      //                              fall below the combine break-even.
      //   kTinyKvNoSplitPages  = 8   max kv_pages <= 8 (<= 1K tokens) never
      //                              splits - the whole KV fits inside one
      //                              combine-overhead window.
      ...
      if (disable_split_kv != 0) {
        ...
        chunk = low;
        // Variable-kv load-balance override.  When kv-lengths span a wide
        // range (one long batch among many short ones), the binary search
        // picks a `chunk` that just fills the grid, leaving the long batch
        // serial on one CTA while short batches finish and idle their SMs.
        // For an imbalanced batch (max/avg >= 1.5) use a smaller chunk
        // (~avg_pages/4 splits per average batch, still >= the 2K-token
        // floor) so the long batch breaks into more parallel slots; the
        // avg/4 target matches the 1-CTA/SM attn kernel.  avg_pages >= 4
        ...
        // The combine kernel caps max_splits at 256 (bounded by its sLSE smem
        // allocation and per-thread LSE-reduction registers).  Round `chunk`
        // up so no batch ever produces more than 256 splits; this only costs
        // parallelism for ultra-long context (the chunk floor rises past
        // kv ~ 512K).
    ...
    // Compute split_counts + parallel inclusive scan.
    int chunks_b = 0;
    int work_slots_b = 0;
    ...
    // max_split_count via cross-warp reduce (covers batch > 32, where a
    // single-warp reduce would undercount).
    ...
    // Write o_indptr and merge_indptr in parallel.
    ...
    // Write info_scalars + s_work_count_shared so the scatter
...
// holds ~240 reg/thread x 512 threads, saturating the register file).
...
py::dict build_decode_schedule(
  ...
  // The single-CTA scheduler keeps per-batch state in shared memory
  // (s_kv_pages[1024], etc.) with one thread per batch and tpb <= 1024, so
  // batch must not exceed 1024; larger batches need a multi-CTA design.
  TORCH_CHECK(batch <= 1024,
              "build_decode_schedule supports batch <= 1024 (got ",
              batch,
              "); larger batches require a multi-CTA scheduler.");
  ...
  // batch x max_pages_global.
  const int64_t max_pages_global = ceil_div<int64_t>(max_seqlen_k, page_size);
  int64_t pad_work = batch * num_q_tiles * std::max<int64_t>(max_pages_global, 1);
  // CUDA-graph capture pads the work-tile count up to the fixed grid the
  // graph was captured with.  The kernel computes
  //   padded_wc = max(work_count, max(1, max_grid_size / num_kv_heads))
  // whenever (enable_cuda_graph && split_kv).  The Python wrapper then does
  // request_indices.narrow(0, 0, padded_work_count) and the tile-scheduler
  // reads that many entries, so the output index arrays must be allocated to
  // at least this padded count.  For small max_seqlen_k the page-based bound
  // (batch * num_q_tiles * max_pages_global) can fall below the graph pad, so
  // take the max here to avoid an out-of-bounds narrow / scheduler read.
  if (enable_cuda_graph) {
    const int64_t graph_pad =
        std::max<int64_t>(1, max_grid_size / std::max<int64_t>(num_kv_heads, 1));
    pad_work = std::max<int64_t>(pad_work, graph_pad);
  }
  ...
  AT_CUDA_CHECK(cudaMemsetAsync(
      request_tensor.data_ptr<int32_t>(), 0, pad_work * sizeof(int32_t), stream));
  AT_CUDA_CHECK(cudaMemsetAsync(
      qo_tile_tensor.data_ptr<int32_t>(), 0, pad_work * sizeof(int32_t), stream));
  AT_CUDA_CHECK(cudaMemsetAsync(
      kv_tile_tensor.data_ptr<int32_t>(), 0, pad_work * sizeof(int32_t), stream));
  AT_CUDA_CHECK(cudaMemsetAsync(
      mask_tensor.data_ptr<int32_t>(), 0, pad_work * sizeof(int32_t), stream));
