...
def prepare_decode_schedule(
    *,
    ...
    # The single-CTA scheduler keeps per-batch state in shared memory, so
    # batch must not exceed 1024; larger batches need a multi-CTA design.
    if int(seqused_k.shape[0]) > 1024:
        raise NotImplementedError(
            "decode schedule supports batch <= 1024 "
            f"(got batch={int(seqused_k.shape[0])}); larger batches require "
            "a multi-CTA scheduler."
        )
    # The two device-data-dependent hang guards (seqused_k[b] >= seqlen_q and
    # the last-partial-page column count) are enforced inside the raw
    # build_decode_schedule() launch wrapper so that direct callers of the raw
    # entrypoint are protected too; see _validate_decode_seqused_k there.
    if int(page_size) <= 0:
    ...
    # max(seqused_k) <= max_seqlen_k is enforced in the raw build_decode_schedule
    # wrapper (single source of truth; protects direct raw callers too).
