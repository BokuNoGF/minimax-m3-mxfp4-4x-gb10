    ...
    int H, int total_rows, int G_total)
{
    ...
    // then strided. 32 lanes hit M r's x (32/M) g's, giving 32/M cache
    ...
    int max_seqlen_q = 0)
{
    ...
    AT_CUDA_CHECK(cudaDeviceGetAttribute(
        &max_smem_per_block, cudaDevAttrMaxSharedMemoryPerBlockOptin, dev));
    if (max_smem_per_block <= 0) {
        AT_CUDA_CHECK(cudaDeviceGetAttribute(
            &max_smem_per_block, cudaDevAttrMaxSharedMemoryPerBlock, dev));
    }
    int max_smem_per_sm = 0;
    AT_CUDA_CHECK(cudaDeviceGetAttribute(
        &max_smem_per_sm, cudaDevAttrMaxSharedMemoryPerMultiprocessor, dev));
    if (max_smem_per_sm <= 0) max_smem_per_sm = max_smem_per_block;

    // -- Pick kWarps per CTA based on device SMEM budget for cursor/hist.
    // SMEM cursor is packed as int16 (2 rows per int32 word).
    int per_warp_smem = ((total_rows + 1) >> 1) * (int)sizeof(int);
    TORCH_CHECK(per_warp_smem <= max_smem_per_block,
                "k2q CSR row count exceeds dynamic shared memory limit");
    int kWarps_pick = 4;
    while (kWarps_pick > 1 &&
           kWarps_pick * per_warp_smem > max_smem_per_block) {
        kWarps_pick >>= 1;
    }
    while (kWarps_pick > 1 &&
           (kWarps_pick * per_warp_smem) * 2 > max_smem_per_sm) {
        kWarps_pick >>= 1;
    }
    if (kWarps_pick < 1) kWarps_pick = 1;

    // -- Pick G (CTAs) ----------------------------------------------------
    // Size one resident wave from actual SM12x shared-memory attributes.
    int per_cta_smem_bytes = kWarps_pick * per_warp_smem;
    int max_ctas_per_sm = std::max(
        1, max_smem_per_sm / std::max(1, per_cta_smem_bytes));
    ...
    if (G < 1) G = 1;
    constexpr int kPackedCounterLimit = 32767;
    int max_q_per_warp_safe = std::max(1, kPackedCounterLimit / kTopK);
    int max_q_per_cta_safe = std::max(1, max_q_per_warp_safe * kWarps_pick);
    int min_g_for_counter = (S_Q + max_q_per_cta_safe - 1) / max_q_per_cta_safe;
    G = std::max(G, min_g_for_counter);
    G = std::min(G, S_Q);
    int q_per_cta = (S_Q + G - 1) / G;
    G = (S_Q + q_per_cta - 1) / q_per_cta;
    int q_per_warp = (q_per_cta + kWarps_pick - 1) / kWarps_pick;
    TORCH_CHECK(q_per_warp * kTopK <= kPackedCounterLimit,
                "k2q CSR per-warp counter would overflow packed int16 storage");
